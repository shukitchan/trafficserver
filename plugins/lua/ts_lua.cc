...
static ts_lua_main_ctx *ts_lua_main_ctx_array   = nullptr;
static ts_lua_main_ctx *ts_lua_g_main_ctx_array = nullptr;
...
create_lua_vms()
{
  ts_lua_main_ctx *ctx_array = nullptr;
...
TSRemapInit(TSRemapInterface *api_info, char *errbuf, int errbuf_size)
{
  ...
  if (nullptr == ts_lua_main_ctx_array) {
    ...
    if (nullptr != ts_lua_main_ctx_array) {
      pthread_key_create(&lua_state_key, nullptr);
...
ts_lua_remap_plugin_init(void *ih, TSHttpTxn rh, TSRemapRequestInfo *rri)
{
  ...
  contp = TSContCreate(ts_lua_http_cont_handler, nullptr);
  ...
  ts_lua_set_cont_info(L, nullptr);
...
vconnHookHandler(TSCont contp, TSEvent event, void *edata)
{
  ...
  if (ret) {
    ...
    Dbg(dbg_ctl, "[%s] no error returned", __FUNCTION__);
  }
  TSVConnReenable(vconn);
  return 0;
}
...
globalHookHandler(TSCont contp, TSEvent event ATS_UNUSED, void *edata)
{
  ...
  txn_contp = TSContCreate(ts_lua_http_cont_handler, nullptr);
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  if (nullptr == ts_lua_g_main_ctx_array) {
    ts_lua_g_main_ctx_array = create_lua_vms();
    if (nullptr != ts_lua_g_main_ctx_array) {
      pthread_key_create(&lua_g_state_key, nullptr);
  ...
  if (ret != 0) {
    TSError("[ts_lua]%.*s", (int)errbuf_len, errbuf);
  ...
  TSCont vconn_contp = TSContCreate(vconnHookHandler, nullptr);
  if (!vconn_contp) {
  ...
  if (lua_type(vl, -1) == LUA_TFUNCTION) {
    ...
    Dbg(dbg_ctl, "vconn_start_hook added");
  }
  lua_pop(vl, 1);
